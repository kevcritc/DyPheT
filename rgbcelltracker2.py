...
class Frame(FileManager):
    ...
    def analyse(self):
        ...
        # self.th = cv2.adaptiveThreshold(self.filtergray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY, self.ath[0], self.ath[1])
        ret,self.th=cv2.threshold(self.filtergray,15,255, cv2.THRESH_BINARY)
        # self.closing = cv2.morphologyEx(self.th, cv2.MORPH_CLOSE, self.kernel)
        ...
        for cnt in self.contours:  
            ...
            if area>self.area_th:
                ...
                if not_colliding(x,y, self.collisionlist):
                    ...
                    cutoutpath=self.filename=os.path.join(self.imagepath, cutoutfile)
...
class Process_cells(OpenVideo):
    def __init__(self, file, path, scale,tpf, maxdistance, addnumbers,search_angle,dpi=800,autotrack=True,w1=1, w2=2):
    ...
    def plotalltracks(self):
        for data in self.datapack:
            ...
            plt.xlabel('x /pixel')
            plt.ylabel('y /pixel')
        figfilename=self.filename[:-4]+'alltracks1.png'
        ...
        for data in self.datapack:
            ...
            plt.xlabel('x /pixel')
            plt.ylabel('y /pixel')
        figfilename=self.filename[:-4]+'alltracks2.png'
        plt.savefig(figfilename)
        plt.show()
        ...
        for i,data in enumerate(self.datapack):
            ...
            plt.xlabel('x /pixel')
            plt.ylabel('y /pixel')
        figfilename=self.filename[:-4]+'alltracks3.png'
        plt.savefig(figfilename)
    ...
    def applyscales(self):
        ...
        del df1
    def write_csv(self):
        csvfilename=self.filename[:-4]+'all_final.csv'
    ...
    def MSDanalysis(self):
        ...
        msddf.to_excel(self.filename[:-4]+'MSD.xlsx')
        figfilename=self.filename[:-4]+'MSD.png'
        plt.savefig(figfilename)
        plt.show()
    ...
    def vectorplot(self, subset='none',colour=True):
        ...
        plt.ylim(Vmax,Vmin)
        plt.axis('equal') 
        figfilename=self.filename[:-4]+'vect1.png'
        plt.savefig(figfilename)
        plt.show()
    ...
    def distance_histogram(self):
        ...
        plt.ylabel('Frequency')
        figfilename=self.filename[:-4]+'dishist.png'
        plt.savefig(figfilename)
        plt.show()
    ...
    def velocityvectorplot(self, subset='none',colour=True):
        ...
        plt.ylim(Vmax,Vmin)
        plt.axis('equal') 
        figfilename=self.filename[:-4]+'vect2.png'
        plt.savefig(figfilename)
        plt.show()    
    ...
    def plot_area_covered(self):
        ...
        plt.ylabel('frequency')
        figfilename=self.filename[:-4]+'areacovered.png'
        plt.savefig(figfilename)
        plt.show()
    def plot_hull_area_covered(self):
        ...
        plt.ylabel('frequency')
        figfilename=self.filename[:-4]+'hullareacovered.png'
        plt.savefig(figfilename)
        plt.show()
